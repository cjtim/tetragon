...
func FindCgroupDir(dir string) (string, error) {
	cgRoot, err := tgcgroups.HostCgroupRoot()
	if err != nil {
		return "", err
	}
	...
	filepath.WalkDir(cgRoot, func(path string, dentry fs.DirEntry, err error) error {
	...
	return filepath.Join(cgRoot, ret), nil
}
...
func TestNamespacedPolicies(t *testing.T) {
	...
	podId2 := uuid.New()
	require.NoError(t, err)
	err = pfState.AddPodContainer(policyfilter.PodID(podId1), "ns1", "pod1-container1", &cgID1)
	require.NoError(t, err)
	err = pfState.AddPodContainer(policyfilter.PodID(podId2), "ns2", "pod1-container2", &cgID2)
	require.NoError(t, err)
	...
	err = sm.Manager.DelTracingPolicy(ctx, "lseek-test")
	require.NoError(t, err)
